        ...
        <Table>
          ...
          <TableBody>
            {isLoading || !l1Data ? (
              ...
              l1Data.map((l1) => (
                <TableRow key={l1.id}>
                  <TableCell className="font-medium text-white">{l1.id}</TableCell>
                  <TableCell>
                    <div className="flex items-center">
                      <div className="w-6 h-6 mr-2 bg-gray-700 rounded-full"></div>
                      <div className="flex flex-col">
                        <span className="text-white">{l1.name}</span>
                        <span className="text-sm text-gray-400">{l1.symbol}</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="text-right text-white">
                    ${l1.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}
                  </TableCell>
                </TableRow>
              ))
            )}
